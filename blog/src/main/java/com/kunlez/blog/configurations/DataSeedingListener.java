...
public class DataSeedingListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void addRoleIfMissing(String name, String description) {
        if (roleRepository.findByName(name) == null) {
            roleRepository.save(new RoleEntity(name, description));
        }
    }

    private void addUserIfMissing(String username, String password, String... roles) {
        if (userRepository.findByUsername(username) == null) {
            UserEntity user = new UserEntity(username, "First name", "Last name", new BCryptPasswordEncoder().encode(password));
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        addUserIfMissing("nguyenduykhanh1025@gmail.com", "nguyenduykhanh1025@gmail.com", "ROLE_MEMBER", "ROLE_ADMIN");
        addUserIfMissing("Khánh705462839908540", "Khánh705462839908540", "ROLE_MEMBER", "ROLE_ADMIN");
